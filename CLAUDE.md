...
**Achado crítico relacionado, ainda EM ABERTO na v1.73 — o Cloudflare Pages serve o repositório inteiro, não só os arquivos do site:** ao investigar o `monitor_web.log`, descobriu-se que `rioiguacu.com/CLAUDE.md` também está publicamente acessível (confirmado 20/07/2026), incluindo o CPF do usuário (seção de acesso à API da ANA). Isso indica que o projeto Cloudflare Pages não tem um "Build output directory" configurado — ele publica a raiz inteira do repositório GitHub como está, então **qualquer arquivo commitado** (CHANGELOG.md, backups/*.zip, scrape.py, app.py, requirements.txt, abrir_monitor_web.bat, cloudflare-worker/*.js, NOTA_TECNICA...pdf) também fica acessível via URL direta em rioiguacu.com/<caminho>, mesmo sem link visível no site. Confirmado também que nem `_headers` nem `_redirects` do Cloudflare Pages conseguem bloquear/retornar 404 pra um arquivo especifico (`_headers` só define cabeçalhos de resposta; `_redirects` não suporta status 404 customizado pra "rewrite") — a única correção real é reestruturar o deploy (ex.: mover os arquivos realmente públicos pra uma subpasta tipo `public/` e configurar essa subpasta como "Build output directory" no painel do Cloudflare Pages, mantendo tudo o mais interno fora dela). Verificado também, via API do GitHub (`git/trees/main?recursive=1`), que não há arquivo de credenciais (`login ana.txt`, `teste_api_ana.py`, etc.) na árvore atual do repositório — mas o histórico completo de commits não foi auditado. **Esta reestruturação ainda não foi implementada** — requer confirmação do usuário antes de mexer na configuração do Cloudflare Pages (mudança de configuração de conta) e reestruturar a árvore do repositório. Não fazer isso "para simplificar" sem essa confirmação.
