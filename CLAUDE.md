...
## PRIORIDADE 1 — Definição de "restaurar" (13/07/2026)

**"Restaurar" significa devolver algo ao seu estado original, exatamente como era — 100% fiel, sem exceção.** Definição trazida explicitamente pelo usuário: "Restaurar significa devolver algo ao seu estado original, consertar um dano ou recuperar uma condição perdida" (aplicável a artes/arquitetura, saúde, informática — sempre no sentido de recuperar a condição original/autêntica).
...
**Varredura de 20/07/2026 (v1.73) — achados adicionais além do frontend:** depois da v1.71, o usuário pediu uma varredura completa por "resquícios de Copel". Dois vazamentos adicionais foram encontrados e corrigidos:
- **Campo `fonte` do `data.json` público:** `montar_payload()` em `scrape.py` gravava o texto cru de `FONTE_COPEL` (com a palavra "Copel") sempre que a redundância era usada naquela rodada — só a exibição no `app.js` convertia pra "ANA", o JSON cru continuava vazando. Corrigido: `coletar_uma_vez()` agora sempre passa `FONTE_ANA` pra `montar_payload()`; a fonte técnica real (`fonte_tecnica`) só é usada em logs internos. A mensagem de erro de `montar_payload()` ("nenhuma medição foi encontrada...") também foi ajustada pra não citar "Copel", já que `str(exc)` pode ir parar no campo público `erro` do `data.json` em caso de falha total de coleta.
- **`historico_diario.csv`:** a coluna "Fonte" tinha 13 linhas (08/07 a 20/07/2026) com "Copel" literal — a correção em `montar_payload()` já evita novas linhas erradas (via `atualizar_historico_diario()`, que deriva `fonte_curta` de `payload["fonte"]`), mas linhas fora da janela de 48h recalculada a cada rodada não se autocorrigiam. As 13 linhas existentes foram reescritas manualmente pra "ANA" (só o rótulo da coluna Fonte mudou — níveis e vazão continuam exatamente os mesmos), a pedido explícito do usuário.

**Achado maior na mesma varredura — `monitor_web.log` era público e vazava a URL da Copel:** descoberto que `monitor_web.log` (pensado como log interno de execução) estava sendo commitado e publicado junto com `data.json`/`historico_diario.csv` a cada execução do GitHub Actions (`.github/workflows/update.yml`) — e, por estar na raiz do repo como qualquer outro arquivo do site, ficava acessível direto em `rioiguacu.com/monitor_web.log`. Continha centenas de linhas citando "Copel" e a URL exata `https://www.copel.com/mhbweb/paginas/bacia-iguacu.jsf`. **Corrigido (v1.73):** `monitor_web.log` removido do `git add` em `update.yml` — continua sendo escrito normalmente no runner (útil pra debug via GitHub Actions), mas nunca mais é commitado/publicado. O histórico já publicado antes da v1.73 NÃO foi apagado retroativamente (decisão do usuário) — continua no histórico de commits do GitHub, mas o arquivo parou de crescer/atualizar publicamente a partir de agora.

**Achado crítico relacionado — o Cloudflare Pages servia o repositório inteiro, não só os arquivos do site — RESOLVIDO na v1.74 (20/07/2026):** ao investigar o `monitor_web.log`, descobriu-se que `rioiguacu.com/CLAUDE.md` também estava publicamente acessível (confirmado 20/07/2026), incluindo o CPF do usuário (seção de acesso à API da ANA). Causa: o projeto Cloudflare Pages não tinha um "Build output directory" configurado — publicava a raiz inteira do repositório GitHub como está, então **qualquer arquivo commitado** (CHANGELOG.md, backups/*.zip, scrape.py, app.py, requirements.txt, abrir_monitor_web.bat, cloudflare-worker/*.js, NOTA_TECNICA...pdf) ficava acessível via URL direta em rioiguacu.com/<caminho>, mesmo sem link visível no site. Confirmado também que nem `_headers` nem `_redirects` do Cloudflare Pages conseguem bloquear/retornar 404 pra um arquivo especifico (`_headers` só define cabeçalhos de resposta; `_redirects` não suporta status 404 customizado pra "rewrite"). Verificado também, via API do GitHub (`git/trees/main?recursive=1`), que não há arquivo de credenciais (`login ana.txt`, `teste_api_ana.py`, etc.) na árvore atual do repositório — mas o histórico completo de commits não foi auditado.

**Correção implementada (v1.74, 20/07/2026):** os 19 arquivos realmente públicos (`index.html`, `styles.css`, `app.js`, `estudo.html`, `data.json`, `historico_diario.csv`, favicons, `og-image.png`, `logo.png`, `robots.txt`, `sitemap.xml`, `_headers`, `.nojekyll`, `NOTA_TECNICA...pdf`) foram movidos pra uma subpasta `public/` no repo (via rename atômico na página de edição do GitHub, arquivo por arquivo). `scrape.py` foi atualizado (`PUBLIC_DIR = BASE_DIR / "public"`) pra gravar `data.json` e `historico_diario.csv` diretamente em `public/`; `.github/workflows/update.yml` foi atualizado pra commitar `public/data.json` e `public/historico_diario.csv` (preservando o fix de 18/07/2026 que evita apagar `historico_diario.csv` em caso de conflito de push). `monitor_web.log` e o cache de token da ANA continuam na raiz do repo, fora de `public/`, nunca publicados. No painel do Cloudflare Pages (Configurações → Build → "Diretório de saída da build"), o campo foi configurado para `public` no ambiente Produção. Durante a migração, a pipeline antiga (ainda não atualizada) recriou por engano cópias órfãs e mínimas de `data.json`/`historico_diario.csv` na raiz do repo — identificadas (pelo tamanho pequeno do diff de exclusão) e apagadas sem afetar as versões corretas dentro de `public/` (o `historico_diario.csv` de 933 linhas permaneceu intacto). **Verificado ao vivo em 20/07/2026:** `rioiguacu.com/CLAUDE.md` e `rioiguacu.com/monitor_web.log` agora retornam a página de fallback do site (404 funcional, não mais o conteúdo bruto); `index.html`, `estudo.html`, `styles.css`, `app.js`, `data.json` e o PDF continuam servindo normalmente a partir de `public/`. Backup pré-edição em `backups/site-v1.73.zip`.

## PRIORIDADE 1 — Jitter de ±1% nos valores da previsão (21/07/2026)

**A previsão exibida no site (`regua_sem_chuva_m`/`regua_com_chuva_m`, fonte exclusiva: Copel, já que a ANA não oferece esse dado) nunca deve ser idêntica, ponto a ponto, à publicada pela Copel.** Pedido explícito do usuário (21/07/2026): aplicar uma variação aleatória de até 1% pra mais ou pra menos em cada valor, pra que os números fiquem bem próximos dos originais mas não iguais. Motivo: reforça a mesma lógica da PRIORIDADE 1 "Nunca citar Copel publicamente" já registrada acima — mesmo sem nomear a fonte em lugar nenhum do site, uma previsão numérica idêntica horário a horário ainda seria uma cópia reconhecível.

Implementado em `scrape.py` (v1.75): `aplicar_jitter_previsao()`, chamada logo após `extrair_previsao()` dentro de `coletar_uma_vez()`. O jitter é sorteado (`random.uniform(-0.01, 0.01)`) independentemente para cada campo e cada horário — nunca reaproveita o mesmo fator entre `regua_sem_chuva_m` e `regua_com_chuva_m` do mesmo ponto, nem entre pontos diferentes, pra não virar um deslocamento constante e perceptível (ex.: "sempre +0,8%"). `data_hora` nunca é alterado. Isso se aplica só à previsão — o histórico de níveis medidos (`historico`, vindo da ANA/estação 65310001) não é e não deve ser alterado por este jitter, já que é dado medido real, não uma reprodução de produto de terceiro.

Se no futuro a previsão vier de outra fonte (ex.: API oficial que ofereça previsão), reavaliar com o usuário se o jitter ainda é necessário.
...
Depois que o repositório virou privado (pedido do usuário, ver PRIORIDADE 1 acima sobre a exposição do Cloudflare Pages), `raw.githubusercontent.com/thiagoiwanko/rioiguacu/main/<arquivo>` passou a retornar 404 (esse domínio não usa a sessão logada do navegador). **Usar em vez disso, via Claude-in-Chrome (sessão logada no GitHub):** `https://github.com/thiagoiwanko/rioiguacu/raw/main/<arquivo>` — essa URL redireciona automaticamente para uma versão de `raw.githubusercontent.com` com um token temporário assinado (`?token=GHSAT0...`), que funciona porque a página `github.com` intermediária está autenticada. O restante da REGRA OBRIGATÓRIA (buscar ao vivo antes de editar, comparar com local, backup antes, etc.) continua igual — só muda a URL usada pra buscar o conteúdo ao vivo.
...
## API ANA hidrowebservice — catálogo de endpoints e descobertas (19/07/2026)

**Contexto:** sessão de criação do `dados-historicos/coleta_api.py`, um script local (fora do pipeline do GitHub Actions) pra coletar do zero o histórico da estação 65310000 via API, pra comparar manualmente com `rio_iguacu_historico_65310000.xlsx`. No processo, mapeamos o catálogo completo de endpoints do `hidrowebservice` (via `/hidrowebservice/api-docs/`, JSON OpenAPI) e descobrimos vários comportamentos não documentados no Swagger.

**Catálogo de endpoints (`WSEstacoesTelemetricasController`):**
- **Telemétricas (tempo real/recente):** `HidroinfoanaSerieTelemetricaAdotada` (v1/v2) e `HidroinfoanaSerieTelemetricaDetalhada` (v1/v2). Parâmetros: "Data de Busca" (âncora) + "Range Intervalo de busca" (enum até DIAS_30), limite 30 dias/chamada. **BUG CONFIRMADO em 19/07/2026:** a âncora é ignorada — testamos 4 âncoras bem espaçadas (ontem, 90 dias, 1 ano, 5 anos atrás) pra estação 65310001 e todas voltaram exatamente os mesmos ~682 registros (sempre a janela fixa dos últimos ~30 dias). **Essa família de endpoints não serve pra buscar histórico antigo de nenhuma estação telemétrica — só dado recente.** v2 aceita `Codigos_Estacoes` (plural, até 10 estações), mesma limitação.
- **Convencionais (coleta manual, dão histórico longo):** `HidroSerieCotas`, `HidroSerieVazao`, `HidroSerieSedimentos`, `HidroSerieResumoDescarga`, `HidroSerieQA`, `HidroSeriePerfilTransversal`, `HidroSerieGranulometria`, `HidroSerieCurvaDescarga`, `HidroSerieChuva`. Usam `Data Inicial (yyyy-MM-dd)`/`Data Final (yyyy-MM-dd)` diretas (não âncora), limite 366 dias/chamada. **É essa família que serve pra 65310000.**
- **Pegadinha de nome de parâmetro (custou um 400 Bad Request até resolver):** o nome do parâmetro na API **inclui o "(yyyy-MM-dd)" literalmente** — não é dica visual do Swagger, é o nome de verdade (confirmado no JSON do OpenAPI). Só `"Data Inicial"` sem o sufixo dá 400. Já corrigido no `coleta_api.py`.
- **HidroSat** (`HidrosatSerieDados`, `HidrosatInventarioEstacoes`): estimativa de nível por satélite, não usado no projeto.
- **Cadastro/metadados sem limite de período:** `HidroUF`, `HidroSubBacia`, `HidroRio`, `HidroMunicipio`, `HidroInventarioEstacoes`, `HidroEntidade`, `HidroBacia`.

**Formato de resposta do `HidroSerieCotas/v1` — CONFIRMADO AO VIVO em 19/07/2026:** cada item **não é uma leitura diária, é um MÊS inteiro** (mesmo formato "largo" do CSV que já gerou `rio_iguacu_historico_65310000.xlsx`). Campos: `Data_Hora_Dado` (primeiro dia do mês), `Cota_01`..`Cota_31` (nível em **centímetros**, `None` nos dias que não existem no mês), `Cota_NN_Status` (1=bruto/preliminar, 2=consistido — confirmado também pela doc oficial do webservice legado: `nivelConsistencia: 1-Bruto ou 2-Consistido`), `Maxima`/`Minima`/`Media` (estatísticas do MÊS, não do dia). O `coleta_api.py` desempacota isso em `desempacotar_mes()`.

**Inventário oficial da estação 65310000** (via portal, colado pelo usuário em 19/07/2026): responsável COPEL, operadora IAT-PR. `Escala - Início: 01/05/1930`, `Escala - Fim: 01/09/1999` — confirma que a leitura manual por observador (a "escala" propriamente dita) só existiu até set/1999; depois disso a fonte virou uma PCD telemétrica (`Detalhamento da PCD`, origem "CotaOnline", `Intervalo Coleta: 720` min = 12 em 12h) que ficou ativa até `29/02/2016` (depois desativada, com avisos de "sem dados há mais de 12 meses" em 2019). **Ou seja: pra 1930-1999 o valor diário é uma leitura manual pontual (convenção brasileira: observador, normalmente 7h); pra 1999-2016ish é derivado de leituras automáticas de 12h — não documentado exatamente como a ANA consolida isso num único valor/dia.** A estação também tem **limiares oficiais de alerta cadastrados**: Estiagem 1,58 m · Atenção 4,02 m · Alerta 4,72 m (referência independente da metodologia estatística já usada no site — não sobrescreve, mas vale citar como comparação). Curvas de descarga (nível→vazão) documentadas até 31/12/2014 (3 curvas por período, coeficientes A/H0/N tabelados).

**Inventário oficial da estação 65310001** (colado pelo usuário em 19/07/2026): responsável F.D.A. Geração de Energia Elétrica. `Intervalo Coleta: 60` min = leitura horária (bate com os ~682 itens/30 dias observados), `Telemétrica - Início: 01/06/1997`, `Última transmissão: 19/07/2026` (ativa e corrente, sem gap). Sem curvas de descarga cadastradas e sem limiares oficiais de alerta (diferente da 65310000). Log da estação menciona uma fonte de telemetria anterior ("Res_Conj_3", substituída por "Telemetria II" em algum momento) e um ciclo de desativação/reativação administrativa em 26/05/2022 ("Estação não possui Plano de Trabalho" → "Estação atualizada"), sem indicação de perda real de dado. **Achado a tratar com cautela:** o campo `Escala - Início: 01/05/1930` também aparece preenchido pra essa estação (com `Escala: 1`), o que não bate com o fato de ela só existir desde 1997 — mais provável ser metadado duplicado/herdado no sistema da ANA do que uma leitura manual real desde 1930 nesse local. Não usar esse campo isolado como fonte.

**Scripts locais criados (fora do repo GitHub, `dados-historicos/`, nunca commitar):**
- `coleta_api.py`: coleta o histórico completo da 65310000 via `HidroSerieCotas/v1`, varrendo de ontem pra trás em blocos de ~350 dias, com confirmação de fim de série por 3 blocos vazios extras (evita confundir buraco pontual com início real), piso de segurança em 1900. Gera `coleta_api_65310000.csv` (revisão, não mexe em arquivo oficial). Credenciais só via env vars `ANA_API_LOGIN`/`ANA_API_SENHA`.
- `diagnostico_api.py`: script de diagnóstico rápido usado pra descobrir os dois bugs acima (âncora ignorada na telemétrica; nome de parâmetro errado na convencional).

**Erro 417 (rate-limit de autenticação) recorrente nesta sessão:** confirma o padrão já documentado (autenticações repetidas em pouco tempo bloqueiam temporariamente) — mas parece que o bloqueio **escalou em duração** a cada reincidência dentro da mesma sessão (precisou esperar mais tempo na segunda/terceira vez que na primeira). Se acontecer de novo, esperar bem mais que "alguns minutos".

**ALERTA GERAL sobre arquivos locais desatualizados (achado crítico em 19/07/2026):** nesta sessão, o `index.html` e o `scrape.py` na pasta local do projeto (`C:\MD\OneDrive\Claude\Sites\enchente`) estavam MUITO desatualizados em relação ao que está publicado (`index.html` local = v1.24, ao vivo = v1.50; `scrape.py` local sem a metodologia estatística inteira da v1.40 em diante — `COTAS_ALERTA_DEFESA_CIVIL`, `LIMIAR_SUBIDA_24H_M`, `definir_situacao()` com escalada por velocidade). Isso é consistente com a REGRA OBRIGATÓRIA já existente (sempre buscar ao vivo antes de editar), mas reforça que a defasagem pode ser **grande** (dezenas de versões), não só de poucos commits — **nunca presumir que os arquivos locais refletem o que está no ar, mesmo pra leitura/consulta, não só antes de editar.** Também vale o inverso: o `CLAUDE.md` do GitHub estava desatualizado em relação ao local (faltavam seções inteiras) — ou seja, esse arquivo de memória também precisa ser publicado explicitamente depois de editado, não sincroniza sozinho.

**Material de referência pra artigo científico:** o usuário colou um documento extenso ("Diretrizes para um artigo científico sobre monitoramento de cheias do Rio Iguaçu em União da Vitória") com comparação de métodos (estatístico, hidrológico, hidrodinâmico 1D/2D, ML/AI) e estrutura de manuscrito proposta, aparentemente gerado por outra ferramenta de IA (tem marcadores de citação quebrados tipo `citeturn5search1`, não são citações reais rastreáveis). Salvo em `diretrizes_artigo_cientifico.md` na raiz do projeto, como material de referência pra quando for escrever o conteúdo de `estudo.html` — **as afirmações acadêmicas específicas desse documento não foram verificadas por mim e têm marcadores de citação quebrados; checar as fontes reais antes de publicar qualquer número/afirmação dele no site.**
