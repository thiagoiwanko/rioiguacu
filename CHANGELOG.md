...
- **Política adotada:** `default-src 'self'`; `script-src`/`style-src` incluem `'unsafe-inline'` — necessário porque `app.js` gera `style="..."` inline com valores calculados em tempo real (hash/nonce não serve pra conteúdo dinâmico) e `estudo.html` tem `onclick=` inline e um bloco `<script>` inline pro seletor de visualização e pro visualizador de PDF; `worker-src` inclui `cdnjs.cloudflare.com` e `blob:` pro Web Worker do pdf.js; `connect-src` inclui o Worker do contador de visitas (`rioiguacu-counter.thiago-dff.workers.dev`); `frame-ancestors 'none'`, `object-src 'none'`, `base-uri 'self'`, `form-action 'self'`. Também adicionados: `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, `Strict-Transport-Security` (1 ano, incluindo subdomínios), `Permissions-Policy` (bloqueia câmera/microfone/geolocalização/pagamento/usb/interest-cohort).
- Backup pré-edição: `backups/_headers-pre-v1.98.bak` (conteúdo original de `public/_headers`).
...
- Backup pré-edição: `backups/site-v1.87-preedicao.zip`.
...
- Backup pré-edição: `backups/site-v1.84-preedicao.zip`.
...
- **Texto "acumulada X mm" um pouco maior** (`#rainAccum`, 14px em vez do tamanho padrão de `.metric-card small`) — pedido do usuário, na mesma conversa.
- Backup pré-edição: `backups/site-v1.82-preedicao.zip`.
